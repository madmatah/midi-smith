...
ImageHeaderParseResult ParseImageHeader(std::span<const std::uint8_t> buffer) noexcept {
  ImageHeaderParseResult result;

  if (buffer.size() < kImageHeaderSizeBytes) {
    result.status = ImageHeaderStatus::kBufferTooSmall;
    return result;
  }

  const std::span<const std::uint8_t> header_bytes = buffer.first(kImageHeaderSizeBytes);
