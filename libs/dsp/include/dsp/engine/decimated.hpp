...
class Decimated {
 ...
 public:
  ...
  float Transform(float input, const ContextT& ctx) noexcept
    requires(midismith::dsp::concepts::DecimatableSignalTransformer<T, ContextT> ||
             midismith::dsp::concepts::SignalTransformer<T, ContextT>)
  {
    if constexpr (midismith::dsp::concepts::DecimatableSignalTransformer<T, ContextT>) {
