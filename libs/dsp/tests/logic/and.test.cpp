...
TEST_CASE("The And struct") {
  using midismith::dsp::logic::And;

  SECTION("The Test() method") {
    SECTION("With two predicates") {
      using Pred = And<Pred1, Pred2>;

      SECTION("When both are true") {
        MockContext ctx{.val1 = true, .val2 = true};
        REQUIRE(Pred::Test(ctx));
      }

      SECTION("When first is false") {
        MockContext ctx{.val1 = false, .val2 = true};
        REQUIRE_FALSE(Pred::Test(ctx));
      }

      SECTION("When second is false") {
    ...
    SECTION("With three predicates") {
      using Pred = And<Pred1, Pred2, Pred3>;

      SECTION("When all are true") {
        MockContext ctx{.val1 = true, .val2 = true, .val3 = true};
        REQUIRE(Pred::Test(ctx));
      }

      SECTION("When one is false") {
        MockContext ctx{.val1 = true, .val2 = false, .val3 = true};
        REQUIRE_FALSE(Pred::Test(ctx));
      }
    }
    ...
    SECTION("Short-circuiting behavior") {
      ...
      SECTION("Should not call subsequent predicates if first is false") {
        MockContext ctx{.val1 = false, .val2 = true, .val3 = true};
