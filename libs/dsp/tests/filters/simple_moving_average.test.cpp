...
using MovingAverage4 = midismith::dsp::filters::SimpleMovingAverage<4u>;
static_assert(midismith::dsp::concepts::SignalTransformer<MovingAverage4, TestContext>);
  ...
  SECTION("The Transform() method") {
    SECTION("When configured with a window size of 1") {
      SECTION("Should return the last input sample") {
        midismith::dsp::filters::SimpleMovingAverage<1u> filter;
        TestContext context{};
    ...
    SECTION("When configured with a window size of 3") {
      SECTION("Should keep only the latest samples after warmup") {
        midismith::dsp::filters::SimpleMovingAverage<3u> filter;
        TestContext context{};
