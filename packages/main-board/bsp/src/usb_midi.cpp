...
domain::midi::TransportStatus UsbMidi::TrySendRawMessage(const uint8_t* data,
                                                         uint8_t length) noexcept {
  if (!IsAvailable()) {
    return domain::midi::TransportStatus::kNotAvailable;
  }

  if (data == nullptr || length == 0 || length > 3) {
    return domain::midi::TransportStatus::kError;
  }
  ...
  if (hmidi == nullptr || hmidi->tx_state != 0) {
    return domain::midi::TransportStatus::kBusy;
  }
  ...
  if (status == USBD_OK) {
    return domain::midi::TransportStatus::kSuccess;
  }
