...
std::span<const std::uint8_t> ApplicationSlotFlash::ApplicationSlot() const noexcept {
  return InternalFlash::ReadRegion(midismith::flash_layout::kApplicationLoadAddress,
...
bool ApplicationSlotFlash::EraseApplicationSlot(std::size_t length_bytes) noexcept {
  return InternalFlash::EraseRegion(midismith::flash_layout::kApplicationLoadAddress, length_bytes);
}

bool ApplicationSlotFlash::ProgramApplicationSlot(std::span<const std::uint8_t> payload) noexcept {
  return InternalFlash::ProgramRegion(midismith::flash_layout::kApplicationLoadAddress, payload);
}
