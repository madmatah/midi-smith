...
TEST_CASE("LogarithmicVelocityMapper") {
  using Mapper =
      midismith::adc_board::domain::music::piano::velocity::LogarithmicVelocityMapper<1.0f, 10.0f>;
  Mapper mapper{};
  ...
  SECTION("Maps an intermediate speed with logarithmic shaping") {
    REQUIRE(mapper.Map(0.5f) == static_cast<midismith::common::domain::music::Velocity>(95u));
  }
}
