...
class LineEditor {
 public:
  ...
  bool Poll(midismith::io::ReadableStreamRequirements& reader,
            midismith::io::WritableStreamRequirements& writer, bool& line_ready,
            CompletionCallback on_completion = nullptr, void* user_data = nullptr) noexcept {
    std::uint8_t byte;
    bool did_rx = false;
    line_ready = false;

    while (reader.Read(byte) == midismith::io::ReadResult::kOk) {
      did_rx = true;
 ...
 private:
  bool ProcessByte(std::uint8_t byte, midismith::io::WritableStreamRequirements& stream,
