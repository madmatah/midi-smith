...
static midismith::main_board::bsp::serial::UartStreamBase* g_streams[kMaxUartStreams] = {nullptr};

static midismith::main_board::bsp::serial::UartStreamBase* FindStream(
  ...
  for (std::size_t i = 0; i < kMaxUartStreams; ++i) {
    midismith::main_board::bsp::serial::UartStreamBase* s = g_streams[i];
...
static void EnsureRegistered(midismith::main_board::bsp::serial::UartStreamBase* stream) noexcept {
  if (stream == nullptr) {
    return;
  }
